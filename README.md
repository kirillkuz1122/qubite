...
- турниры с разными режимами допуска, включая отображение текущих, будущих и завершённых соревнований в пользовательской витрине; кодовые и roster-турниры по умолчанию скрываются из общей витрины;
...
- уведомление о cookies/localStorage и базовые legal pages для RU-контура.
...
- OAuth-провайдеры в текущем коде: `Google`, `Yandex`, `VK ID`, `Telegram Login Widget`;
- CAPTCHA/anti-bot в текущем коде: `Cloudflare Turnstile`;
- deploy-модель: `Nginx + PM2 + SQLite + Cloudflare`;
- Telegram-бот для управления платформой (owner + модераторы): тумблеры, включая NaiveProxy, аналитика, модерация, админка, push-уведомления.
...
- Telegram login (бот управления есть, login через TG — нет).
...
Отдельные способы OAuth-входа owner может скрывать без рестарта через админку или Telegram-тумблеры. Там же есть runtime-тумблер `proxy_naive_enabled`: при выключении NaiveProxy не выдаётся в списке серверов, catalog и subscription links, а sync отдаёт пустой список Naive credentials, оставляя VLESS-профили. Env-переменные `OAUTH_GOOGLE_ENABLED=false`, `OAUTH_YANDEX_ENABLED=false`, `OAUTH_VK_ENABLED=false`, `OAUTH_TELEGRAM_ENABLED=false` остаются аварийными kill-switch'ами.
