...
Если в локальной среде уже настроен `Nginx`, можно работать через него, как в production-like режиме. Но для большинства задач достаточно прямого `node server.js` из каталога `back/`.
...
- `proxy_naive_enabled` — runtime-тумблер в `system_settings`: при `false` NaiveProxy скрывается из `/api/proxy/servers`, `normal` в `/api/proxy/catalog`, subscription links и sync credentials; VLESS/SNI остаётся включённым.
