...
6. Пользовательская витрина турниров сохраняет завершённые соревнования в списке, чтобы вкладка "Прошедшие" работала и после досрочного завершения.
7. Для `code` и `closed`-допуска сервер дополнительно проверяет `catalog_visible`: скрытые турниры не попадают в общую витрину, кроме случая roster-доступа текущего пользователя.
